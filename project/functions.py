...
def convert_data_from_google_sheet_to_db_format() -> list[tuple[int, int, float, datetime.date]]:
    """
    ...
    converted_data_from_google_sheet = [(int(object_from_table_google_sheets[0]), 
            ...
            if len(object_from_table_google_sheets) == 4 and data_format_check_from_google_sheets(object_from_table_google_sheets[0], 
...
def get_dollar_exchange_rate()  -> float:
    ...
    raise Exception ('Необходимо проверить источник XML c ресурса ЦБ РФ, получаемое значение не является курсом доллара США')
...
def get_data_from_db_for_template() -> list[tuple]:
    ...
    """
    data_from_db_for_template = db.session.query(TableGoogleSheets.item_number, 
                                    TableGoogleSheets.order_number, 
                                    TableGoogleSheets.price_dollar,
...
def calculate_total_dollar() -> float:
    ...
    """
    list_table_googlesheets= db.session.query(TableGoogleSheets.price_dollar).all()
    total_dollar = 0
    for object in list_table_googlesheets:
...
def calculate_total_rub() -> float:
    ...
    """
    list_table_googlesheets= db.session.query(TableGoogleSheets.price_ruble).all()
    total_rub = 0
    for object in list_table_googlesheets:
        total_rub += object[0]
